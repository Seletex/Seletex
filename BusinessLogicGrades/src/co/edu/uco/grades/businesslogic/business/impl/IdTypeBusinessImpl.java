...
public class IdTypeBusinessImpl implements IdTypeBusiness{
	...
	public IdTypeBusinessImpl(DAOFactory daoFactory) {
		if(UtilObject.getUtilObject().isNull(daoFactory)) {
			throw GradesException.buildTechnicalBusinessLogicException("It's not possible create a IdTypeBusinessLogicImpl when the DAOFactory is null");
		}
		
		this.daoFactory = daoFactory;
	}

	public void validateIdTypeDoesNotExistingWithSameName(IdTypeDTO dto) {
		...
		if(!list.isEmpty()) {
			var message = "An IdType with the same already exists";
	...
	public void create(IdTypeDTO dto) {
		validateIdTypeDoesNotExistingWithSameName(dto);
		daoFactory.getIdTypeDAO().create(dto);
