...
public class IdTypeAzureSqlDAO extends ConnectionSQL implements IdTypeDAO {
	...
	public List<IdTypeDTO> find(IdTypeDTO idType) {
		...
		sb.append("SELEECT id,name ").append(SPACE);
		...
		try (PreparedStatement preparedStatement = getConnection().prepareStatement(sb.toString())){	
			...
			throw GradesException.buildTechnicalDataException("There was a problem trying to retrieve the IdType on Azure SQL Server", exception);
		} catch (Exception exception) {
			throw GradesException.buildTechnicalDataException("An unexpected problem has ocurred trying to retrieve the IdType on Azure SQL Server", exception);
		}
	...
	private List<IdTypeDTO> assembleResults(ResultSet resultSet){
		...
		try {	
			...
			while(resultSet.next()){
				...
				throw GradesException.buildTechnicalDataException("There was a problem trying to recover the IdType on Azure SQL Server", exception);
			} catch (Exception exception) {
				throw GradesException.buildTechnicalDataException("An unexpected problem has ocurred trying to recover the IdType on Azure SQL Server", exception);
			}
